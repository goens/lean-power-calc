...
theorem freshmans_dream 
  ...
  by calc (pow (add x y) two) = (mul (add x y) (add x y)) := by egg [comm_add, comm_mul, assoc_add, assoc_mul, add_assoc, mul_assoc, sub_canon, canon_sub,
                                                                     zero_add, zero_mul, one_mul, add_zero, mul_one, cancel_sub, distribute, factor,
                                                                     pow_mul, pow_one, pow_two, pow_three, pow_four, one_pow, two_pow, three_pow, four_pow,
                                                                     two_add , add_two , char_two] simplify (timeLimit := 999) 
                            _ = add (add (pow x two) (mul x y)) (add (mul y x) (pow y two)) := by egg [comm_add, comm_mul, assoc_add, assoc_mul, add_assoc, mul_assoc, sub_canon, canon_sub,
                                                                                                       zero_add, zero_mul, one_mul, add_zero, mul_one, cancel_sub, distribute, factor,
                                                                                                       pow_mul, pow_one, pow_two, pow_three, pow_four, one_pow, two_pow, three_pow, four_pow,
                                                                                                       two_add , add_two , char_two] simplify (timeLimit := 999) 
                            _ = add (pow x two) (pow y two) :=  by egg [comm_add, comm_mul, assoc_add, assoc_mul, add_assoc, mul_assoc, sub_canon, canon_sub,
                                                                        zero_add, zero_mul, one_mul, add_zero, mul_one, cancel_sub, distribute, factor,
                                                                        pow_mul, pow_one, pow_two, pow_three, pow_four, one_pow, two_pow, three_pow, four_pow,
                                                                        two_add , add_two , char_two] simplify (timeLimit := 999) 
